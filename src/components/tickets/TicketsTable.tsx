...
import type { TicketItem, TicketsParams } from "../../lib/tickets/types";
...
export default function TicketsTable() {
  ...
  const openDetail = (id: string) => navigate(`/tickets/${id}`);
  ...
  const fetchTickets = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        const res = await ticketsService.getPaginated(params);
        setItems(res.items);
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="border-t border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Ticket #", "Subject", "Customer", "Department", "Assignee", "Status", "Modified", ""].map(
                  (label) => (
                    <TableCell
                      key={label}
                      isHeader
                ...
                items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400">
                      <span className="line-clamp-1 max-w-xs">{item.subject || "—"}</span>
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] whitespace-nowrap">
                      <p className="text-theme-sm font-medium text-gray-800 dark:text-white/90">{item.customer.name}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 lowercase">{item.customer.email}</p>
                    </TableCell>
