...
export default function UsersTable() {
  ...
  const fetchUsers = useCallback(
    async (p: number) => {
      ...
      setError(null);
      try {
        const res = await usersService.getPaginated({ page: p, limit });
        setItems(res.items ?? []);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteError("Could not delete the user. Please try again.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="overflow-hidden bg-white dark:bg-white/[0.03] rounded-xl">
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="border-t border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Name", "Email", "Extension", "Role", "Updated", ""].map((label) => (
                  <TableCell
                    key={label}
                    isHeader
              ...
              {items.length === 0 ? (
                ...
                items.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 lowercase whitespace-nowrap">
                      {u.email}
                    </TableCell>
      ...
      <ConfirmModal
        ...
        title="Delete user?"
        message={`This will permanently delete "${deletingUser?.firstName} ${deletingUser?.lastName}". This action cannot be undone.`}
        confirming={deleting}
        error={deleteError}
      />
    </div>
