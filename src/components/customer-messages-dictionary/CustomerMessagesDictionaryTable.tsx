...
import CustomerMessageDictionaryFormModal from "./CustomerMessageDictionaryFormModal";
...
export default function CustomerMessagesDictionaryTable() {
  ...
  const [editingEntry, setEditingEntry] = useState<CustomerMessageDictionaryItem | null>(null);

  const [deletingEntry, setDeletingEntry] = useState<CustomerMessageDictionaryItem | null>(null);
  ...
  const fetchEntries = useCallback(
    async (p: number) => {
      ...
      try {
        ...
        setItems([]);
        setError("Failed to load the status dictionary. Please try again.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const openEdit = (entry: CustomerMessageDictionaryItem) => {
  ...
  const handleDelete = async () => {
    ...
    setDeleteError(null);
    try {
      await customerMessagesDictionaryService.remove(deletingEntry.id);
      setDeletingEntry(null);
      fetchEntries(page);
    } catch {
      setDeleteError("Could not delete this status. Please try again.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="overflow-hidden bg-white dark:bg-white/[0.03] rounded-xl">
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="border-t border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Step", "Display Name", "Internal Name", "SMS Code", "Updated", ""].map((label) => (
                  <TableCell
                    key={label}
                    isHeader
                ...
                items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm font-medium text-gray-800 dark:text-white/90 whitespace-nowrap">
                      {item.order_status_display_name}
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                      {item.order_status_internal_name}
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                      {item.sms_code || "—"}
                    </TableCell>
      ...
      <CustomerMessageDictionaryFormModal
        isOpen={formOpen}
      ...
      <ConfirmModal
        ...
        title="Delete status?"
        message={`This will permanently delete "${deletingEntry?.order_status_display_name}". This action cannot be undone.`}
        confirming={deleting}
        error={deleteError}
      />
    </div>
