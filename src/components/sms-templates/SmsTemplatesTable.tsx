...
export default function SmsTemplatesTable() {
  ...
  const [items, setItems] = useState<SmsTemplateItem[]>([]);
  ...
  const fetchTemplates = useCallback(
    async (p: number) => {
      ...
      setError(null);
      try {
        const res = await smsTemplatesService.getPaginated({
          page: p,
          limit,
          ...(query && { query }),
          ...(active && { active: active as "yes" | "no" }),
        });
        setItems(res.items ?? []);
        ...
        setItems([]);
        setError("Failed to load SMS templates. Please try again.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const openEdit = (template: SmsTemplateItem) => {
    setEditingTemplate(template);
    setFormOpen(true);
  };

  const toggleActive = async (template: SmsTemplateItem, next: boolean) => {
    setItems((prev) =>
      prev.map((t) => (t.id === template.id ? { ...t, active: next ? "yes" : "no" } : t))
    );
    try {
      await smsTemplatesService.setActive(template.id, next);
    } catch {
      // Revert on failure
      setItems((prev) =>
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="border-t border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["ID", "Name", "Message", "Updated", "Updated By", "Active", ""].map((label) => (
                  <TableCell
                    key={label}
                    isHeader
              ...
              {items.length === 0 ? (
                <TableRow>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                    No SMS templates found for the selected filters.
                  </td>
                </TableRow>
              ) : (
                items.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400">
                      <span className="line-clamp-1 max-w-sm">{t.message_body}</span>
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
