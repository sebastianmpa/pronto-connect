...
export default function PermissionsTable() {
  ...
  const [deletingPermission, setDeletingPermission] = useState<PermissionItem | null>(null);
  ...
  const fetchPermissions = useCallback(
    async (p: number) => {
      ...
      setError(null);
      try {
        const res = await permissionsService.getPaginated({ page: p, limit });
        setItems(res.items ?? []);
  ...
  const handleDelete = async () => {
    ...
    setDeleteError(null);
    try {
      await permissionsService.remove(deletingPermission.id);
      setDeletingPermission(null);
      fetchPermissions(page);
    } catch {
      setDeleteError("Could not delete the permission. Please try again.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="overflow-hidden bg-white dark:bg-white/[0.03] rounded-xl">
                ...
                items.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400">
                      <span className="line-clamp-1 max-w-sm">{p.description}</span>
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
        ...
        <div className="border border-t-0 rounded-b-xl border-gray-100 py-4 pl-[18px] pr-4 dark:border-white/[0.05]">
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
            ...
            <PaginationWithIcon
              ...
              initialPage={page}
              onPageChange={(p) => fetchPermissions(p)}
            />
          </div>
        </div>
      )}
      ...
      <ConfirmModal
        ...
        title="Delete permission?"
        message={`This will permanently delete "${deletingPermission?.name}". This action cannot be undone.`}
        confirming={deleting}
        error={deleteError}
      />
    </div>
