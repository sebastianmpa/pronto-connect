...
interface ContactReasonFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Present when editing an existing reason; omitted when creating a new one. */
  reason?: ContactReasonItem | null;
  onSaved: () => void;
}

export default function ContactReasonFormModal({
  isOpen,
  onClose,
  reason,
  onSaved,
}: ContactReasonFormModalProps) {
  ...
  const handleSave = async () => {
    if (!reasonName.trim() || !description.trim()) {
      setError("Reason name and description are both required.");
      return;
    }
    setSaving(true);
    setError(null);
    try {
      if (isEditing && reason) {
        await contactReasonsService.update(reason.id, { reasonName, description });
      } else {
        await contactReasonsService.create({ reasonName, description });
      }
      onSaved();
      onClose();
    } catch {
      setError("Could not save the contact reason. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div>
        <h4 className="text-title-sm mb-1 font-semibold text-gray-800 dark:text-white/90">
          {isEditing ? `Edit Contact Reason — ${reason?.internalName}` : "New Contact Reason"}
        </h4>
        <p className="mb-6 text-sm leading-6 text-gray-500 dark:text-gray-400">
          The internal name is generated automatically from the reason name and can&apos;t be edited.
        </p>
        ...
        <div className="mt-8 flex flex-col sm:flex-row w-full items-center justify-between gap-3">
          ...
          >
            {saving ? "Saving…" : isEditing ? "Save changes" : "Create reason"}
          </button>
        </div>
      </div>
    </Modal>
