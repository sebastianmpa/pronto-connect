...
import ClosureMethodFormModal from "./ClosureMethodFormModal";
...
export default function ClosureMethodsTable() {
  const [limit, setLimit] = useState(10);
  const [page, setPage] = useState(1);

  const [items, setItems] = useState<ClosureMethodItem[]>([]);
  ...
  const [editingMethod, setEditingMethod] = useState<ClosureMethodItem | null>(null);
  ...
  const fetchMethods = useCallback(
    async (p: number) => {
      ...
      setError(null);
      try {
        const res = await closureMethodsService.getPaginated({ page: p, limit });
        setItems(res.items ?? []);
        ...
        setItems([]);
        setError("Failed to load closure methods. Please try again.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const openEdit = (method: ClosureMethodItem) => {
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="border-t border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Method Name", "Internal Name", "Description", "Updated", ""].map((label) => (
                  <TableCell
                    key={label}
                    isHeader
                ...
                items.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400">
                      <span className="line-clamp-1 max-w-sm">{m.description}</span>
                    </TableCell>
                    <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-theme-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
        ...
        <div className="border border-t-0 rounded-b-xl border-gray-100 py-4 pl-[18px] pr-4 dark:border-white/[0.05]">
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
            ...
            <PaginationWithIcon
              ...
              initialPage={page}
              onPageChange={(p) => fetchMethods(p)}
            />
          </div>
        </div>
      )}

      <ClosureMethodFormModal
        isOpen={formOpen}
        onClose={() => setFormOpen(false)}
        method={editingMethod}
        onSaved={() => fetchMethods(page)}
      />
    </div>
  );
}
